...
As a comparison to the above *Table 1*, I have simulated the distribution of 372,899 
...
Comparing the *Table 1* and *Table 2* of relative portions of major crimes, I observed that 
...
Now, I will display the progression of the number of annual major crime reports made over the 10-year period. I used ggplot2 [@ggplotcite] to generate *Figure 1* with the years from 2014 to 2023, inclusive, in chronological order on the x-axis, which more clearly illustrates the trend of major crime occurrences over time.
...
## Tail Analysis of Neighbourhoods Affected By All Five Major Crimes
...
neighbourhoods in which the most major crimes were reported in the former five 
years with the 10 most affected neighbourhoods in the latter five years, shown in *Table 3* and *Table 4* below.
...
kable(table_data_3, caption = "**Top 10 Neighbourhoods with Most Major 
...
kable(table_data_4, caption = "**Top 10 Neighbourhoods with Most Major 
...
It is important to examine both ends of the neighbourhood crime spectrum to understand more about the distribution of reported crimes across Toronto neighbuorhoods. Now I will observe whether the safest neighbourhoods in the former five years of the time period also remain the safest neighbourhoods in the latter five years, or if they differ materially.
...
kable(table_data_3, caption = "**The 10 Neighbourhoods with the Least Major 
      Crimes Reported from 2014 to 2018, inclusive**")
...
kable(table_data_4, caption = "**The 10 Neighbourhoods with the Least Major 
      Crimes Reported from 2019 to 2023, inclusive**")

```

I observed from the two tables above, that 7 of the 10 neighbourhoods least 
...
As an aside; I consulted ChatGPT 3.5 to assist in generating Tables 3, 4, 5, and 6, along with the total in each table [@chatcite].

## Tail Analysis of Neighbourhoods Affected By Violent Crimes (Assaults)
...
as shown below in Table 7 and Table 8.
...
Once again, we must examine both ends of the spectrum, and so I will display the results in Table 9 and Table 10 for the 10 neighbourhoods least affected by assaults in the first five years, and then the 10 neighbourhoods least affected by assaults in the latter five years of the last decade.
...
I observed from Table 1 and Table 2 above, that 6 of the 10 neighbourhoods least affected 
by assaults in the first half of the last decade also carry over as the least 
affected neighbourhoods in the second half of the last decade. This figure can be 
compared to the previously compiled number of 8 of the 10 neighbourhoods most affected 
by assaults also carrying over to the second half of the decade.

As an aside; I consulted ChatGPT 3.5 to assist in generating Tables 7, 8, 9, and 10, along with the total in each table [@chatcite].

## Comparison of Results in Neighbourhoods Affected by All Major Crimes (MCs) and only Violent Crimes (VCs)

When comparing the progression of the 10 neighbourhoods most affected by major crimes (MCs) over the time period of 2014 to 2023, inclusive, to the 10 neighbourhoods least affected by MCs over the same time period, I noticed that while 3 of the least affected neighbourhoods in 
the first 5 years were replaced by other neighbourhoods whose MC rates reduced 
...
is that those same 10 neighbourhoods were still the most affected even in the latter 5 
...
only 2 neighbourhoods were replaced moving into the second half of the decade, whereas for 
neighbourhoods least affected by VCs, 4 of the neighbourhoods were replaced moving 
...
To analyze this relationship, I generated a graph, Figure 2, which measures the correlation between 
two variables. The first variable is the number of major crimes reported annually 
in Toronto, as shown in Figure 1. The second variable is the annual 
...
In Figure 2 below, I consulted ChatGPT 3.5 to include the line of best fit and the calculated linear correlation coefficient [@chatcite]. 
...
of Toronto's public policy may have a slower effect on impacting a meaningful reduction in the number of crimes occurring in Toronto neighbourhoods, which could mean that perhaps in the next 5 or 10 years, we may observe more material impacts in the reduction of major crimes or specifically violent crimes in high-risk neighbourhoods
...
This paper analyzes the Toronto Police Services’ success in materially reducing the frequency of major crime indicators in high-risk neighbourhoods across Toronto from 2014 to 2023, inclusive. Data analysis shows that the Toronto Police Service has **not** materially reduced the frequency of major crime indicators in the last 10 years, instead showing a correlation between stagnating major crime indicator frequency in low-risk neighbourhoods. This correlation points to the unrest which exists in the bedrock of the top 10 high-risk neighbourhoods as a result of socio-economic, demographic and built-environment characteristics, suggesting that the complex systems which underscore police-reported crime and the performance of violent crime itself revolve deeply around large- and small-scale factors, events and phenomena.
